...
## The principle — just-in-time context

skill-rag is **just-in-time context** applied to *skills*: stage only what the current prompt needs,
keep the rest at zero cost. Auto-staging every prompt is safe **here** precisely because it's
bounded and self-cleaning — it stages a handful of name+description lines and removes the previous
turn's, so it can never snowball into the context window.

The same instinct applied to *knowledge* points the other way: whole notes are heavy, so you **pull**
them when a task needs them rather than auto-injecting them every turn. Different mechanics, same
goal — the context window only ever holds what the moment actually requires.
